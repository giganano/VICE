...
with Forced Explosions

**Signature**: from vice.yields.ccsne import LC18M
...
if not __VICE_SETUP__:
	...
	def set_params(**kwargs):
		...
		**Signature**: vice.yields.ccsne.LC18M.set_params(\*\*kwargs)
		...
		* TypeError
			- 	Received a keyword argument "study". This will always be
				"LC18M" when called from this module.
		...
		------------
		>>> import vice
		>>> from vice.yields.ccsne import LC18M
		>>> vice.yields.ccsne.settings['o']
		0.0036512768277795864
		>>> LC18M.set_params(IMF = "salpeter")
		>>> vice.yields.ccsne.settings['o']
		0.0022556257770960713
		>>> LC18M.set_params(rotation = 150)
		>>> vice.yields.ccsne.settings['o']
		0.010851696623329273
		>>> LC18M.set_params(rotation = 150, IMF = "salpeter")
		>>> vice.yields.ccsne.settings['o']
		0.006744726174198793
		>>> LC18M.set_params(rotation = 150, IMF = "salpeter", MoverH = -2)
		>>> vice.yields.ccsne.settings['o']
		0.0081235534350932
		...
		if "study" in kwargs.keys():
			...
			for i in range(len(_RECOGNIZED_ELEMENTS_)):
				__settings[_RECOGNIZED_ELEMENTS_[i]] = __fractional(
					_RECOGNIZED_ELEMENTS_[i], study = "LC18M", **kwargs)[0]
