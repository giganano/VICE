...
class mlr_generator: 
	...
	r""" 
	A callable object designed to run calculations of the main sequence mass 
	fraction for various assumptions about the mass-lifetime relation. 
	A step size of 1.e-4 is used to ensure that the assumed MLR responds 
	...
	def __call__(self): 
		def test(): 
			try: 
				...
				times = [1.e-4 * i for i in range(10001)] 
				status = True 
				for time in times: 
				...
				return False 
			return status 
		return ["vice.core.main_sequence_mass_fraction [MLR :: %s]" % (
			self.mlr), test] 
