...
class output(object):
	...
	def show(self, key):
		...
		try:
			...
			message = b"Error: could not import python package matplotlib. "
			message += b"Please install matplotlib and/or anaconda and "
			message += b"try again."
			raise ImportError(message)

		_mpl.set_params()
		fig = plt.figure(figsize = (7, 7))
		ax = fig.add_subplot(111, facecolor = b"white")
		# ax.ticklabel_format(useOffset = False, style = "plain")
		_mpl.set_frame(ax)

		if key.lower() in self._history.labels:
			ax.set_xlabel(b"t [Gyr]")
			ax.set_ylabel(key)
			ax.plot(self._history[b"time"][1:], self._history[key.lower()][1:], 
				c = b'k')
			...
			ax.set_xlabel(key[4:])
			ax.set_ylabel(key)
			xvals = map(lambda x, y: (x + y)/2., self._mdf[b"bin_edge_left"], 
				self._mdf[b"bin_edge_right"])
			ax.step(xvals, self._mdf[key.lower()], c = b'k')
			...
			if self.__new_key_mdf(key) in self._mdf.labels:
				...
				xvals = map(lambda x, y: (-x + -y)/2., 
					self._mdf[b"bin_edge_left"], 
					self._mdf[b"bin_edge_right"])
				ax.step(xvals, self._mdf[self.__new_key_mdf(key)], c = b'k')
			else:
				message = b"Metallicity distribution function not found in "
				message += b"output: %s" % (key)
				raise ValueError(message)
		elif key.lower() == b"tau_star":
			tstar = map(lambda x, y: 1.e-9 * x / y, self._history[b"mgas"], 
				self._history[b"sfr"])
			ax.set_xlabel(b"t [Gyr]") 
			ax.set_ylabel(r"$\tau_{*} = M_{gas}/\dot{M}_{*}$ [Gyr]")
			ax.plot(self._history[b"time"][1:], tstar[1:], c = b'k')
		elif key.lower() == b"eta":
			eta = map(lambda x, y: x / y, self._history[b"ofr"], 
				self._history[b"sfr"])
			ax.set_xlabel(b"t [Gyr]")
			ax.set_ylabel(r"$\eta = \dot{M}_{out}/\dot{M}_{*}$")
			ax.plot(self._history[b"time"][1:], eta[1:], c = b'k')
		elif '-' in key.lower():
			if len(key.split(b'-')) != 2:
				message = b"Can only show tracks in 2-dimensional space.\n"
				message += b"Key: %s" % (key)
				raise ValueError(message)
			else:
				abundance_y = key.split(b'-')[0]
				abundance_x = key.split(b'-')[1]
			...
			plt.clf()
			del plt
			raise KeyError(b"Unrecognized dataframe key: %s" % (key))
	...
	def __history_columns(self):
		...
		"""
		with open(b"%s/history.out" % (self._name), b'r') as f:
			line = f.readline()
			while line[0] == b'#' and line[:17] != b"# COLUMN NUMBERS:":
				line = f.readline()
			if line[0] == b'#':
				labels = []
				while line[0] == b'#':
				...
				f.close()
				message = b"Output history file appears to not be formatted "
				message += b"correctly: %s/history.out" % (self._name)
	...
	def __mdf_columns(self):
		...
		"""
		with open(b"%s/mdf.out" % (self._name), b'r') as f:
			line = f.readline().split()
			f.close()
			if line[0] == b'#':
				return tuple(map(lambda x: x.lower(), line[1:]))
			else:
				message = b"Output MDF file appears to not be formatted "
				message += b"correctly."
				raise IOError(message)

	def __flip_XoverY_mdf(self, key):
		if key[:4].lower() == b"dn/d":
			return True
		else:
			return False

	def __new_key_mdf(self, key):
		element1 = key[4:].split(b'/')[0][1:]
		element2 = key[4:].split(b'/')[1][:-1]
		return b"dn/d[%s/%s]" % (element2.lower(), element1.lower())

	def __new_key_history(self, key):
		try:
			element1 = key.split(b'/')[0][1:]
			element2 = key.split(b'/')[1][:-1]
			return b"[%s/%s]" % (element2.lower(), element1.lower())
		except IndexError:
			return None
...
class _dataframe(object):
		...
		if isinstance(value, str):
			if value.lower() in self._frame:
				...
				try:
					...
					raise KeyError(b"Invalid dataframe key: %s" % (value))
		elif isinstance(value, numbers.Number):
			if value % 1 == 0:
				if value >= 0 and value < len(self._frame[self._labels[0]]):
					return [self._frame[i][value] for i in self._labels]
				else:
					raise IndexError(b"Index out of range: %d" % (value))
			else:
				raise ValueError(b"Indexing by number must be a valid integer.")
		else:
			message = b"Index must be either a string or integer. Got: %s" % (
				type(value))
	...
	def __setitem__(self, key, value):
		try:
			...
			message = b"Setting an item must be an array-like object of the "
			message += b"same length as the dataframe itself."
			raise TypeError(message)
		if all(map(lambda x: isinstance(x, numbers.Numbers), value)):
			if len(value) == len(self._frame[self._labels[0]]):
				if isinstance(key, str):
					...
					message = b"Can only set item based on column with key "
					message += b"of type string."
					raise TypeError(message)
			else:
				message = b"Mismatch in array size: must match that of "
				message += b"dataframe. Dataframe size: %d. Got: %d" % (
					len(self._frame[self._labels[0]]), len(value))
				raise ValueError(message)
		else:
			raise TypeError(b"Non-numerical value detected.")
	...
	def read(self, labels):
		...
		if contents == NULL:
			raise IOError(b"File not found: %s" % (self._filename))
		else:
			...
			if len(labels) != dim:
				raise ValueError(b"Must have a label for each dimension.")
			else:
	...
	def __XoverY(self, key):
		element1 = key.split(b'/')[0][1:]
		element2 = key.split(b'/')[1][:-1]
		if b"[%s/h]" % (element1.lower()) not in self._labels:
			raise ValueError
		elif b"[%s/h]" % (element2.lower()) not in self._labels:
			raise ValueError
		else:
			return map(lambda x, y: x - y, 
				self._frame[b"[%s/h]" % (element1.lower())], 
				self._frame[b"[%s/h]" % (element2.lower())]
			)
