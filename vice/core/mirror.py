	...
	if isinstance(arg, strcomp): 
		...
		if _is_multizone(arg): 
			...
			if params["migration.stars"] != None: 
				mz.migration.stars = params["migration.stars"] 
			else: 
				warnings.warn("""\
...
def from_path(filename): 
	...
	""" 
	# let error handling occur in output.__init__ 
...
instead be set to the default value: """
			...
			# filter out entrainment settings 
			keys = list(filter(lambda x: not x.startswith("entrainment"), 
				params.keys())) 
			copy = dict(zip(keys, [params[i] for i in keys])) 
			sz = singlezone(**copy) 
			for i in _RECOGNIZED_ELEMENTS_: 
				sz.entrainment.agb[i] = params["entrainment.agb"][i] 
