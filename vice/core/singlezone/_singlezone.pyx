...
# callable function. Got: %s""" % (type(value))) 
			raise TypeError("Attribute 'IMF' must be a string. Got: %s" % (
				type(value))) 
...
be lost.\nOutput directory: %s.vice\nOverwrite? (y | n) """ % (self.name)) 
	...
	def setup_elements(self): 
		...
		for i in range(self._sz[0].n_elements): 
			...
			# Code masked for future compatability 
			self._sz[0].elements[i][0].agb_grid[0].entrainment = 1 
			self._sz[0].elements[i][0].ccsne_yields[0].entrainment = 1 
			self._sz[0].elements[i][0].sneia_yields[0].entrainment = 1 
