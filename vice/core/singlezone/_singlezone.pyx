...
simulation.\nOutput directory: %s.vice\nOverwite? (y | n) """ % (self.name))
	...
	def setup_elements(self):
		...
		"""
		from ...elements import nonmetals, destroyed_by_stars
		_nonmetals = nonmetals()
		_destroyed = destroyed_by_stars()
		...
		for i in range(self._sz[0].n_elements):
			...
			self._sz[0].elements[i][0].destroyed_by_stars = <unsigned short> int(
				self.elements[i] in _destroyed)
