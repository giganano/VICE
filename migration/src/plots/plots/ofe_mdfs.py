...
def setup_axes(): 
	...
	for i in range(3): 
		for j in range(5): 
			...
			if i == 0: axes[i][j].set_title("%g - %g kpc" % (
					[3, 5, 7, 9, 11][j], [5, 7, 9, 11, 13][j]), 
				fontsize = 25) 
			if j == 0: axes[i][j].text(0.0, 0.2, 
...
def main(name, imgstem): 
	...
	plt.savefig("%s_ofe_mdfs.pdf" % (imgstem)) 
	plt.savefig("%s_ofe_mdfs.png" % (imgstem)) 
	plt.clf() 
