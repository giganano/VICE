...
def plot_subsample(ax, h277, min_rgal, max_rgal, min_age, max_age, 
	...
	# 	"range": 		[0, 20] 
	# } 
	# if len(stars["id"]) > 2000: 
	# 	kwargs["bins"] = 1000 
	# elif len(stars["id"]) > 200: 
	# 	kwargs["bins"] = 100 
	# else: 
	# 	return 
	# pdf, bins = np.histogram(stars[plot], **kwargs) 
...
def setup_axes(): 
	...
	# axes[0][0].set_ylabel(r"PDF(Birth $R_\text{gal}$)") 
	# axes[1][0].set_ylabel(r"PDF(Final $R_\text{gal}$)") 
	return axes 
