...
class sfe: 
	...
	# Sigma_g1 = 2.e7 # Msun kpc^-2 
	# Sigma_g2 = 5.e6 # Msun kpc^-2 
	...
	**kwargs : real numbers 
	...
	def __init__(self, area, present_day_molecular = 2, molecular_index = 0.5, 
		Sigma_g1 = 2.0e+07, Sigma_g2 = 5.0e+06, index1 = 3.6, index2 = 1.7): 
		self.area = area 
	...
	def __call__(self, time, sfr): 
		...
		Sigma_sfr1 = self.Sigma_g1 / molecular 
		Sigma_sfr2 = (self.Sigma_g2 / self.Sigma_g1)**(
				self._index1 * (1 - self._index1) / 
				(self._index1 - self._index2) 
			) / molecular * self.Sigma_g1**(
		...
		if Sigma_sfr >= Sigma_sfr1: 
			return molecular 
		elif Sigma_sfr2 <= Sigma_sfr <= Sigma_sfr1: 
			return molecular**(1 / (1 - self._index1)) * (
				Sigma_sfr / self.Sigma_g1
			)**(self._index1 / (1 - self._index1)) 
		elif Sigma_sfr: 
			return (self.Sigma_g2 / self.Sigma_g1)**(
				self._index1 / (1 - self._index2) 
			) * molecular**(1 / (1 - self._index2)) * (
				Sigma_sfr / self.Sigma_g2 
			)**(self._index2 / (1 - self._index2)) 
		else: 
			# avoid ZeroDivisionError if sfr == 0 
			return 1.e-12 # VICE enforces this minimum gas mass anyway 
	...
	def molecular(self, time): 
		...
		return self.present_day_molecular * (
			(1 + time) / 13.7)**self.molecular_index 
	...
	def molecular_index(self): 
		...
		t = 12.7 Gyr is equal to the value of the attribute 
	...
	def Sigma_g2(self): 
		...
		index of the attribute ``index1``. This power-law index will be 
	...
	def index1(self): 
		...
		the gas surface densities of the attributes ``Sigma_g2`` and 
		``Sigma_g1``. 
	...
	def index2(self): 
		...
		gas surface density of the attribute ``Sigma_g2``. 
